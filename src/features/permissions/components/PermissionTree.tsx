      ...
      <div className="space-y-3">
        <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm">
          ...
          <Button
            type="button"
            variant="ghost"
            className="h-auto p-0 font-medium text-muted-foreground underline-offset-4 hover:bg-transparent hover:text-foreground hover:underline"
            onClick={collapseAll}
          >
            Thu gọn tất cả
          </Button>
                ...
                <div className="flex items-center gap-2 border-b border-border bg-muted/40 px-3 py-2.5 md:px-4">
                  ...
                  <label className="flex min-w-0 flex-1 cursor-pointer items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(v) => {
                        checkboxForm.setValue(`module:${modId}`, v === true)
                        ...
                        return (
                          ...
                          >
                            <FormField
                              ...
                              name={`leaf:${cid}`}
                              render={({ field }) => (
                                <FormItem className="m-0 space-y-0 border-0 p-0">
                                  <FormControl>
                                    <Checkbox
                                      className="mt-0.5"
                            ...
                            <div>
                              ...
                              <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground">
                                Giới hạn phạm vi dữ liệu (ABAC) — backend áp dụng khi có truy vấn
                      ...
                      return (
                        <li key={cid} className="flex items-center gap-2.5">
                          <FormField
                            ...
                            name={`leaf:${cid}`}
                            render={({ field }) => (
                              <FormItem className="m-0 flex items-center space-y-0 border-0 p-0">
                                <FormControl>
                                  <Checkbox
                                    checked={Boolean(field.value)}
                                    disabled={disabled}
